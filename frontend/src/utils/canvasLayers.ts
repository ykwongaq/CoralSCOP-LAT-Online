...
import { getLabelColor, getTextColor } from "../components/common/LabelColorMap";
...
export async function buildLayers(data: Data): Promise<LayersResult> {
	const width =
	...
	for (let annIdx = 0; annIdx < data.annotations.length; annIdx++) {
		...
		for (let i = 0; i < pixelMask.length; i++) {
			...
			md[idx] = r; md[idx + 1] = g; md[idx + 2] = b; md[idx + 3] = 255;
			...
			if (isEdge) {
				bd[idx] = r; bd[idx + 1] = g; bd[idx + 2] = b; bd[idx + 3] = 255;
			}
		...
		if (count > 0) {
			centroids.push({ cx: Math.round(sumX / count), cy: Math.round(sumY / count), labelId: ann.labelId });
		}
	}
	...
	for (const { cx, cy, labelId } of centroids) {
		...
		const adjFontSize = displayText.length > 1 ? Math.floor(fontSize * 0.75) : fontSize;
		textCtx.font = `bold ${adjFontSize}px Arial`;
		textCtx.fillStyle = textColor;
		textCtx.fillText(displayText, cx, cy);
	}

	return { layers: { mask: maskCanvas, border: borderCanvas, text: textCanvas }, pixelMasks };
}
...
export function hitTestMask(mask: Uint8Array, width: number, imgX: number, imgY: number): boolean {
	const px = Math.floor(imgX);
	const py = Math.floor(imgY);
	if (px < 0 || py < 0 || px >= width || py >= mask.length / width) return false;
